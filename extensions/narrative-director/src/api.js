const NarrativeDirectorApi = (() => {
  ...
  function createApi(marinara) {
    ...
    async function updateChatActivation(chatId, active) { const statuses = await getAgentStatuses(chatId); if (active && Object.values(statuses).some((entry) => entry.status === "missing")) throw new Error("Import both fixed Narrative Director agents before activation."); const chat = await getChat(chatId); const metadata = core.parseMetadata(chat?.metadata); return patch(`/chats/${encodeURIComponent(chatId)}/metadata`, { activeAgentIds: core.updateFixedActivation(core.activeAgentTypes(metadata), active), ...(active ? { enableAgents: true } : {}) }); }
    ...
    async function analyzeStory(connectionId, sourceText, rules) {
      ...
      const response = await rewrite({ connectionId, selectedText: sourceText, instruction: core.ANALYSIS_PROMPT, agentName: "Narrative Director Story Extractor", dataLabel: "Fictional source", contextSections: [{ label: "User analysis rules", content: core.analysisRules(rules) }] }, "Analysis");
    ...
    async function initializeFromChat(connectionId, project, messages, options = {}) {
      ...
      const response = await rewrite({ connectionId, selectedText: prepared.input, instruction: core.INITIALIZATION_PROMPT, agentName: "Narrative Director Chat Initializer", dataLabel: "Private project and recent active history" }, "Initialization");
    ...
    async function discoverProjectLorebook(chatId) { const books = await listLorebooks(); const candidates = books.filter((book) => book?.chatId === chatId || book?.scope?.mode === "specific" && Array.isArray(book.scope.chatIds) && book.scope.chatIds.includes(chatId)); for (const book of candidates) { if (!core.cleanId(book?.id)) continue; const entries = await listLorebookEntries(book.id); if (entries.some((entry) => entry?.name === core.DIRECTOR_LOREBOOK_SENTINEL)) return { lorebookId: book.id, entries }; } return null; }
    ...
    async function syncProjectLorebook(project) {
      ...
      if (!lorebookId) { const created = await createLorebook({ name: `${project.name} · Agent Transport`, description: "Narrative Director v3 agent transport. Do not select as a Knowledge Router source.", category: "uncategorized", chatId: project.chatId, scope: { mode: "specific", chatIds: [project.chatId] }, generatedBy: "user", recursiveScanning: false, excludeFromVectorization: true }); lorebookId = core.cleanId(created?.id); if (!lorebookId) throw new Error("No lorebook ID returned."); }
      await keepTransportInactive(project.chatId, lorebookId); if (!entries.length) entries = await listLorebookEntries(lorebookId); const transport = core.buildLorebookTransport(project); const directorKnown = entries.find((row) => row.name === core.DIRECTOR_LOREBOOK_SENTINEL) || entries.find((row) => row.id === project.agentEntryIds?.director); const trackerKnown = entries.find((row) => row.name === core.TRACKER_LOREBOOK_SENTINEL) || entries.find((row) => row.id === project.agentEntryIds?.tracker); const director = await upsertEntry(lorebookId, directorKnown, transport.director); const entryIds = { director: director.id, tracker: "" };
      try { const tracker = await upsertEntry(lorebookId, trackerKnown, transport.tracker); entryIds.tracker = tracker.id; } catch (error) { error.directorLorebookUpdated = true; error.lorebookId = lorebookId; error.entryIds = entryIds; throw error; }
