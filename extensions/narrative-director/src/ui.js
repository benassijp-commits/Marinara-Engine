(() => {
        ...
        <main class="nd-main"><div class="nd-empty" data-slot="empty"><h2>Build a private story plan</h2><p>Extract facts first, review disclosure, then compile public resources locally.</p><button class="nd-button nd-button-primary" type="button" data-action="new">Create project</button></div>
          <form class="nd-editor" data-slot="editor" hidden>
            ...
            <section class="nd-panel" data-panel="private" hidden>
              <div class="nd-intro"><strong>Director-only project</strong><span>Saved to Director memory only after explicit synchronization.</span></div>
            ...
            <section class="nd-panel" data-panel="initialize" hidden>
              ...
              <div class="nd-inline-actions" data-slot="initialization-actions" hidden><button class="nd-button nd-button-primary" type="button" data-action="confirm-initialization">Confirm and synchronize memory</button><button class="nd-button nd-button-quiet" type="button" data-action="discard-initialization">Discard</button></div>
            </section>
            <section class="nd-panel" data-panel="agents" hidden>
              ...
              <div class="nd-warning" data-slot="memory-contract-warning">Marinara persists custom-agent memory, but the current generic custom-agent pipeline does not inject it into AgentContext. Synchronization is available for backup/recovery; roleplay consumption requires upstream support.</div>
              <div class="nd-inline-actions"><button class="nd-button nd-button-primary" type="button" data-action="sync-memory">Synchronize both memories</button><button class="nd-button nd-button-quiet" type="button" data-action="load-memory">Load project from Director memory</button><button class="nd-button nd-button-quiet" type="button" data-action="refresh-agents">Refresh status</button></div>
  ...
  async function confirmInitialization() { if (!state.initializationProposal) return; const project = readDraft(); const updated = core.createProject({ ...project, confirmedInitialState: state.initializationProposal }, project.createdAt); setBusy(true); try { await api.syncProjectMemories(updated); const saved = core.createProject({ ...updated, memorySyncedChatId: updated.chatId, memorySyncedAt: new Date().toISOString() }, updated.createdAt); await storage.saveProject(saved); state.draft = saved; state.projects = await storage.listProjects(); state.initializationProposal = null; renderInitialization(null); toast("State confirmed and memories synchronized.", "success"); } catch (error) { errors([error.message, "The previous confirmed state remains unchanged."]); } finally { setBusy(false); } }
  ...
  async function syncMemory() { const project = readDraft(); if (!project) return; setBusy(true); try { await api.syncProjectMemories(project); const saved = core.createProject({ ...project, memorySyncedChatId: project.chatId, memorySyncedAt: new Date().toISOString() }, project.createdAt); await storage.saveProject(saved); state.draft = saved; state.projects = await storage.listProjects(); toast("Director and Tracker memories synchronized.", "success"); } catch (error) { errors([error.message, error.directorMemoryUpdated ? "Director memory updated, Tracker memory failed. Retry synchronization." : "No confirmed local state was discarded."]); } finally { setBusy(false); } }
  async function loadMemory() { const project = readDraft(); if (!project?.chatId) return errors(["Choose a chat."]); setBusy(true); try { const memories = await api.loadProjectMemories(project.chatId); const recovered = core.recoverProjectFromDirectorMemory(memories.director, { chatId: project.chatId, analysisConnectionId: project.analysisConnectionId, initializationConnectionId: project.initializationConnectionId }); state.currentId = recovered.id; state.draft = recovered; state.dirty = true; writeDraft(recovered); toast("Project recovered from Director memory. Save locally to keep this draft.", "success"); } catch (error) { errors([error.message]); } finally { setBusy(false); } }
  async function activate(active) { const project = readDraft(); if (!project?.chatId) return errors(["Choose a chat."]); setBusy(true); try { await api.updateChatActivation(project.chatId, active); await refreshAgents(); toast(active ? "Fixed agents activated. Existing agents preserved." : "Fixed agents deactivated. Memory was not deleted.", "success"); } catch (error) { errors([error.message]); } finally { setBusy(false); } }
  ...
  marinara.on(root, "click", (event) => {
    ...
    if (action === "sync-memory") void syncMemory(); if (action === "load-memory") void loadMemory(); if (action === "refresh-agents") void refreshAgents();
