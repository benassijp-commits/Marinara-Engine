(() => {
  ...
  async function analyze() {
    ...
    try {
      const analysis = await api.analyzeStory(before.analysisConnectionId, before.sourceText, before.projectType, { signal: controller.signal, resume: state.analysisCheckpoint, onProgress: (progress) => { progressPanel.hidden = false; const label = $('[data-slot="analysis-progress-label"]'); if (progress.subdivided) label.textContent = `Block ${progress.blockPath} was truncated and subdivided automatically.`; else if (progress.autoSubdivided) label.textContent = `Analyzing subdivided block ${progress.blockPath} (${progress.block} of ${progress.blockCount})`; else label.textContent = `Analyzing block ${progress.block} of ${progress.blockCount}`; $('[data-slot="analysis-progress-range"]').textContent = `Characters ${progress.charStart.toLocaleString()}–${progress.charEnd.toLocaleString()}`; } });
