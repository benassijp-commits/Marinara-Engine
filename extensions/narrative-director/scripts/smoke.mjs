...
project = core.createProject({ ...project, confirmedInitialState: initial }, project.createdAt); const director = core.buildDirectorDocument(project); const tracker = core.buildTrackerPlan(project); assert.match(JSON.stringify(director), /refuge/); assert.doesNotMatch(JSON.stringify(tracker), /refuge|route is shown/i);
const transport = core.buildLorebookTransport(project); assert.equal(transport.director.name, core.DIRECTOR_LOREBOOK_SENTINEL); assert.equal(transport.tracker.name, core.TRACKER_LOREBOOK_SENTINEL); assert.deepEqual(transport.director.keys, [core.NEVER_MATCH_REGEX]); assert.equal(new RegExp(core.NEVER_MATCH_REGEX).test("ordinary roleplay"), false); assert.doesNotMatch(transport.tracker.content, /refuge|route is shown/i);
