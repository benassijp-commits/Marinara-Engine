...
export const CURATOR_TRACKER_PROMPT = [
  ...
  "Weight your sources in this order when they disagree: the bible first — it is the intended guide for the story, not just a record, so a storyline's or relationship's written direction outranks anything that seems to contradict it. The chat summary is second — a reference of what has actually happened so far. The handful of most recent messages come last — they show only the current moment, the story's live trajectory, not an authority that overrides an established fact or direction. If that trajectory is drifting away from where the bible's storylines or relationships say it should be going, that drift is exactly what driftNote exists to catch and correct — don't just silently follow it.",
  "When deciding how a character feels or behaves, treat the bible's direction as the default to follow — never replace it with an alternate feeling just because that alternate would seem realistic or psychologically plausible given the current scene or summary. Only report something that contradicts the bible's direction when the character's own words or actions in the transcript demonstrate it unambiguously — never because it 'would make sense' for them to feel that way given the moment. This applies every turn, not only when you notice an explicit conflict.",
...
export const CURATOR_SCENE_PROMPT = [
  ...
  "Always prioritize the bible over the chat summary and over the current scene when deciding a character's disposition — bible first, summary second, scene last, every time, not only when you notice them conflicting. A character's disposition in the state block already reflects that ordering; trust it as their real current baseline even if the last message or two seems to lean a different way. Never write a line that contradicts the bible's prescribed direction just because the current scene or summary would make that reading plausible or realistic — only diverge from the bible's direction when the character's own words or actions in the recent messages demonstrate it unambiguously.",
