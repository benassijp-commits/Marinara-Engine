...
export const CURATOR_SCENE_PROMPT = [
  ...
  "When either trigger fires, write a line with concrete named facts and terms from the secret's truth field — real vocabulary, not an abstract gesture like \"the full scope of it\" or \"the truth about their world\", which gives the narrator nothing to actually write with and forces it to invent its own terminology. You don't have to hand over the entire truth in one line — reveal only as much as is proportional to what this exact moment calls for (the first layer of a multi-part truth is enough if that's all the scene needs right now); the rest can come in a later turn as the conversation continues. This is the one case where naming real content is correct instead of forbidden. If neither trigger fires, keep giving only the effects-not-causes hint as usual — most turns should stay in that mode.",
  ...
  "Every time you deliver a secret's content (from either trigger above), pair it with an explicit note of who present does NOT have this knowledge — by name, not just implied. This is what stops the narrator from defaulting to treating a partial reveal as universal: NPCs left to their own judgment tend to write as if anything already said in the story is common knowledge to everyone, since they have no built-in sense of who was or wasn't in a given scene. Spell it out: e.g. \"Scion does not know this — if Damian chooses to reveal it, this is what he'd be revealing, only to Scion, not to anyone else present who doesn't already know it.\" Do this even when it feels redundant; it is the actual mechanism that keeps a partial reveal from silently becoming a universal one.",
